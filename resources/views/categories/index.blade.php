...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="{{ route('categories.create') }}" class="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 transition duration-200">
            + Add New Category
        </a>
    </div>
    ...
    <div class="mb-8">
        <form method="GET" action="{{ route('categories.index') }}" class="flex gap-4">
            ...
            @if(request('search'))
                <a href="{{ route('categories.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition duration-200">
                    Clear
                </a>
            @endif
        </form>
            ...
            @foreach($categories as $category)
                <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                    ...
                    <div class="relative">
                        ...
                        @else
                            <div class="h-48 w-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center">
    ...
    @else
        <div class="text-center py-12">
            ...
            <p class="text-gray-500 mb-6">
                @if(request('search'))
                    No categories match your search criteria. Try different keywords or <a href="{{ route('categories.index') }}" class="text-blue-600 hover:underline">clear the search</a>.
                @else
                    Get started by adding your first category.
                @endif
            </p>
            <a href="{{ route('categories.create') }}" class="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 transition duration-200">
                + Add First Category
            </a>
        </div>
    @endif
    ...
    <div class="mt-8 flex justify-center">
        <a href="{{ route('products.index') }}" class="bg-gray-600 text-white px-6 py-3 rounded-lg hover:bg-gray-700 transition duration-200 font-semibold">
            ← Back to Products
        </a>
    </div>
</div>
